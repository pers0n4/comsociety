...
const Directions = () => {
  ...
  const [kakaoMap, setKakaoMap] = React.useState();
  const [kakaoMarker, setKakaoMarker] = React.useState();
  ...
  //   kakaoRvClient.getNearestPanoId(position, 50, function (panoId) {
  ...
  //     // 로드뷰가 함께 표시되게 합니다
  //     if (kakaoContainer.className.indexOf('view_roadview') === -1) {
  //       kakaoContainer.className = 'view_roadview';
  ...
  //   var rvPosition = kakaoRv.getPosition();

  //   // 지도의 중심을 현재 로드뷰의 위치로 설정합니다
  //   kakaoMap.setCenter(rvPosition);

  //   // 지도 위에 로드뷰 도로 오버레이가 추가된 상태이면
  //   if (isOverlayOn) {
  //     // 마커의 위치를 현재 로드뷰의 위치로 설정합니다
  //     kakaoMarker.setPosition(rvPosition);
  //   }
  // };
  ...
  //   setKakaoRvClient(rvClient);

  //   // 로드뷰에 좌표가 바뀌었을 때 발생하는 이벤트를 등록합니다
  //   kakao.maps.event.addListener(rv, 'position_changed', handleChangePosition);
  ...
  return (
    <div id="container">
      <div id="rvWrapper">
        <div id="roadview" style={{ height: '100%', width: '100%' }}></div>
        {/* <div id="close" title="로드뷰닫기" onClick={closeRoadview}> */}
        <div id="close" title="로드뷰닫기">
          <span class="img"></span>
        </div>
      </div>
      <div id="mapWrapper">
